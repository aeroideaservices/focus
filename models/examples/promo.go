...
type Promo struct {
	...
	Code                string               `focus:"title:Код;shownInList;disabled:update;unique;sortable;filterable" validate:"required,pattern=sluggable"`
	Title               string               `focus:"title:Название;sortable;filterable;position:2;viewExtra:slugifyCode" validate:"required"`
	...
	RedirectLink        string               `focus:"title:Ссылка для перехода (акция без детальной страницы);hidden:list" validate:"omitempty,pattern=url"`
	IncludedKladrIds    array.StringArray    `focus:"title:Включенные КЛАДР;view:kladrSelect;viewExtra:selectKladrIds;hidden:list" validate:"unique"`
	...
	InfoEmail           string               `focus:"title:Контактный email;view:emailInput" validate:"omitempty,pattern=email"`
	InfoPhone           string               `focus:"title:Контактный телефон;view:phoneInput" validate:"omitempty,pattern=phone"`
	...
	FilteredCatalogLink string               `focus:"title:Ссылка на отфильтрованный каталог;hidden:list" validate:"omitempty,pattern=url"`
